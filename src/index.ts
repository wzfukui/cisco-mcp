  ...
  try {
    switch (name) {
      ...
      case 'connect_cisco_device': {
        const { deviceAlias } = args as { deviceAlias: string };
        const dev = deviceConfigMap.get(deviceAlias);
        if (!dev) throw new Error(`Unknown deviceAlias ${deviceAlias}`);
        const res = await connectionManager.connect(dev);
        return { content: [{ type: 'text', text: JSON.stringify(res, null, 2) }] };
      }
      case 'execute_cisco_command': {
        const { deviceAlias, command, mode = 'user' } = args as {
          deviceAlias: string;
          command: string;
        ...
        try {
          ...
          if (msg.startsWith('No active connection')) {
            const dev = deviceConfigMap.get(deviceAlias);
            if (!dev) throw new Error(`${msg} and deviceAlias not found in config`);
            const connRes = await connectionManager.connect(dev);
            if (!connRes.success) throw new Error(`Auto-connect failed: ${connRes.message}`);
            output = await connectionManager.executeCommand(deviceAlias, command, mode);
          } else {
            throw err;
          }
